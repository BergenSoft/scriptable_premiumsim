...
Version 1.0.6:
    - Fixed login as Drillisch has changed the login page.
...
async function getDataUsage()
{
    ...
    if (req.response.statusCode == 302 && req.response.headers["Location"] == "/start")
    {
        ...
        if (dataInclusive.indexOf('GB') !== -1)
            ...
            dataInclusive = parseInt(dataInclusive.substr(0, dataInclusive.length - 2) / 1024, 10);
        else if (dataInclusive.indexOf('KB') !== -1)
            dataInclusive = parseInt(dataInclusive.substr(0, dataInclusive.length - 2) / 1024 / 1024, 10);
        else if (dataInclusive.indexOf('B') !== -1)
            dataInclusive = parseInt(dataInclusive.substr(0, dataInclusive.length - 1) / 1024 / 1024 / 1024, 10);
